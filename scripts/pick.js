...
/**
 * pick.js — Interactive component picker for claude-sfdx-iq
 *
 ...
 * Outputs a custom manifest JSON that can be used with `claude-sfdx-iq install --manifest`.
 *
 * Usage:
 *   npx claude-sfdx-iq pick                             # Interactive picker
 *   npx claude-sfdx-iq pick --category apex             # Filter to apex domain
 *   npx claude-sfdx-iq pick --search "testing"          # Search by keyword
 *   npx claude-sfdx-iq pick --output manifests/my.json  # Save manifest to file
 *   npx claude-sfdx-iq pick --non-interactive           # Print component catalog as JSON
 */
...
async function runPicker(options = {}) {
  ...
  console.log('\n  ╔══════════════════════════════════════════╗');
  console.log('  ║    claude-sfdx-iq Component Picker       ║');
  console.log('  ╚══════════════════════════════════════════╝');
  ...
  const manifest = {
    name: 'custom',
    description: 'Custom manifest generated by claude-sfdx-iq pick',
...
if (require.main === module) {
  runPicker().then(manifest => {
    ...
    if (outputPath) {
      ...
      console.log(`  Install with: npx claude-sfdx-iq install --manifest ${outputPath}\n`);
    } else {
      console.log('  Generated manifest:\n');
      console.log(json);
      console.log('  To save: npx claude-sfdx-iq pick --output manifests/custom.json\n');
    }
  }).catch(err => {
