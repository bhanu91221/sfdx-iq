 ...
 * Outputs a custom manifest JSON that can be used with `csiq install --manifest`.
 *
 * Usage:
 *   npx csiq pick                             # Interactive picker
 *   npx csiq pick --category apex             # Filter to apex domain
 *   npx csiq pick --search "testing"          # Search by keyword
 *   npx csiq pick --output manifests/my.json  # Save manifest to file
 *   npx csiq pick --non-interactive           # Print component catalog as JSON
 */
...
async function runPicker(options = {}) {
  ...
  const manifest = {
    name: 'custom',
    description: 'Custom manifest generated by csiq pick',
...
if (require.main === module) {
  runPicker().then(manifest => {
    ...
    if (outputPath) {
      ...
      console.log(`  Install with: npx csiq install --manifest ${outputPath}\n`);
    } else {
      console.log('  Generated manifest:\n');
      console.log(json);
      console.log('  To save: npx csiq pick --output manifests/custom.json\n');
    }
  }).catch(err => {
