...
/**
 * csiq — CLI entry point for sfdx-iq plugin
 *
 * Usage:
 *   npx sfdx-iq install [--profile <name>] [--dry-run]
 *   npx sfdx-iq status
 *   npx sfdx-iq list
 *   npx sfdx-iq doctor
 *   npx sfdx-iq repair
...
function showHelp() {
  showBanner();
  console.log('Usage: npx sfdx-iq <command> [options]\n');
  ...
  console.log('\nExamples:');
  console.log('  npx sfdx-iq setup-project');
  console.log('  npx sfdx-iq install --profile developer');
  console.log('  npx sfdx-iq install --profile minimal --dry-run');
  console.log('  npx sfdx-iq status');
  console.log('  npx sfdx-iq doctor');
  console.log('  npx sfdx-iq pick --category apex');
  console.log('  npx sfdx-iq pick --output manifests/custom.json');
  console.log('  npx sfdx-iq install --interactive');
  console.log('  npx sfdx-iq tokens --profile default');
  console.log('  npx sfdx-iq tokens --all');
  console.log('  npx sfdx-iq refresh --profile minimal');
  console.log('');
}
...
if (!COMMANDS[command]) {
  ...
  console.error('Run "npx sfdx-iq help" for usage information.');
  process.exit(1);
}
...
try {
  ...
  if (err.code === 'MODULE_NOT_FOUND') {
    ...
    console.error('Run "npx sfdx-iq repair" to fix missing files.');
    process.exit(1);
  }
  throw err;
}
